...
(async()=>{
  ...
  const queueWindow={GraviStorageSchema:{DB_NAME:"gravi-evidence-v1",DB_VERSION:3,FILE_STORE:"evidenceFiles",QUEUE_STORE:"evidenceQueue",REPORT_STORE:"incidentReports",OPERATIONAL_STORE:"operationalCache"}};vm.runInNewContext(queue,{window:queueWindow,console,queueMicrotask});const check=queueWindow.GraviOfflineEvidenceQueue.isAbandoned,now=Date.parse("2026-07-15T12:05:00Z");assert.equal(check({status:"uploading",updatedAt:"2026-07-15T12:02:59Z"},120000,now),true);assert.equal(check({status:"uploading",updatedAt:"2026-07-15T12:04:00Z"},120000,now),false);assert.equal(check({status:"pending",updatedAt:"2026-07-15T12:00:00Z"},120000,now),false);
