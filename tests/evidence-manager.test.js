...
const root=path.resolve(__dirname,".."),source=fs.readFileSync(path.join(root,"src/evidence-manager.js"),"utf8"),queueSource=fs.readFileSync(path.join(root,"src/offline-evidence-queue.js"),"utf8"),storageSource=fs.readFileSync(path.join(root,"src/system-storage.js"),"utf8"),legacy=fs.readFileSync(path.join(root,"src/legacy-capture-adapter.js"),"utf8"),sql=fs.readFileSync(path.join(root,"database/preventive_controls_evidence_v2.sql"),"utf8");
const listeners={},document={createElement:null,querySelector:()=>null,body:{append(){},classList:{add(){},remove(){}}}},window={crypto:webcrypto,navigator:{onLine:false},addEventListener:(name,fn)=>{listeners[name]=fn},GraviStorageSchema:{DB_NAME:"gravi-evidence-v1",DB_VERSION:3,FILE_STORE:"evidenceFiles",QUEUE_STORE:"evidenceQueue",REPORT_STORE:"incidentReports",OPERATIONAL_STORE:"operationalCache"},GraviLegacyCaptureAdapter:{hasAvailablePhoto:item=>/^data:image\/(jpeg|png|webp);base64,/i.test(item?.photo||"")}};
...
(async()=>{
  ...
  assert.ok(!source.includes("localStorage"),"El administrador no debe guardar blobs en localStorage");assert.ok(storageSource.includes('FILE_STORE:"evidenceFiles"')&&storageSource.includes('QUEUE_STORE:"evidenceQueue"')&&queueSource.includes("GraviStorageSchema"));
