(function createGraviSupabase(global){
  ...
  const workEvidence=Object.freeze({list:async(workId="")=>{requirePermission("evidence.view");if(!configured||!currentSession?.access_token)return[];return request("work_evidence",{query:`?deleted_at=is.null${workId?`&work_id=eq.${encodeURIComponent(workId)}`:""}&select=*&order=created_at.desc`});},create:item=>createPreventiveEvidence({...item,recordType:"work_evidence"}),update:async(id,item)=>{requirePermission("evidence.upload");const rows=await request("work_evidence",{method:"PATCH",query:`?id=eq.${encodeURIComponent(id)}`,headers:{Prefer:"return=representation"},body:item});return Array.isArray(rows)?rows[0]:rows;}});
