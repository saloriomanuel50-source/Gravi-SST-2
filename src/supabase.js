(function createGraviSupabase(global){
  ...
  const preventiveControlRow=item=>({id:item.id,client_uuid:item.client_uuid,work_id:item.work_id||item.workId,development_id:item.development_id||item.developmentId||null,folio:item.folio||null,control_type:item.control_type||item.controlType,description:item.description||"",activity:item.activity||null,area:item.area||null,contractor_id:item.contractor_id||item.contractorId||null,responsible_id:item.responsible_id||item.responsibleId||null,commitment_date:item.commitment_date||item.commitmentDate||null,status:item.status||"draft",priority:item.priority||"medium",immediate_action:item.immediate_action||item.immediateAction||"",work_suspended:Boolean(item.work_suspended??item.workSuspended),area_secured:Boolean(item.area_secured??item.areaSecured),source_module:item.source_module||item.sourceModule||"capture-center",source_entry_id:item.source_entry_id||item.sourceEntryId||null,legacy_source:item.legacy_source||item.legacySource||null,sync_status:item.sync_status||"synced",deleted_at:item.deleted_at||null,created_by:item.created_by||currentSession?.user?.id||null,created_at:item.created_at||new Date().toISOString(),updated_at:new Date().toISOString()});
  ...
  async function createPreventiveEvidence(item){requirePermission("evidence.upload","Tu perfil no permite guardar evidencia.");const table=item.recordType==="work_evidence"?"work_evidence":"preventive_control_evidence",body=item.recordType==="work_evidence"?{id:item.id,client_uuid:item.client_uuid,work_id:item.workId||item.work_id,description:item.caption||item.description||"",evidence_date:item.createdAt||item.created_at||new Date().toISOString(),storage_path:item.storage_path,mime_type:item.mimeType||item.mime_type,size_bytes:item.sizeBytes||item.size_bytes||0,source_module:item.sourceModule||item.source_module||"capture-center",source_entry_id:item.sourceEntryId||item.source_entry_id||null,status:"active",created_by:currentSession.user.id,metadata:item.metadata||{}}:{id:item.id,client_uuid:item.client_uuid,control_id:item.control_id,evidence_stage:item.evidence_stage||"general",storage_path:item.storage_path,caption:item.caption||"",mime_type:item.mimeType||item.mime_type,size_bytes:item.sizeBytes||item.size_bytes||0,width:item.width||null,height:item.height||null,metadata:item.metadata||{},created_by:currentSession.user.id};const rows=await request(table,{method:"POST",query:"?on_conflict=client_uuid",headers:{Prefer:"resolution=merge-duplicates,return=representation"},body});return Array.isArray(rows)?rows[0]:rows;}
  ...
  const workEvidence=Object.freeze({list:async(workId="")=>{requirePermission("evidence.view");if(!configured||!currentSession?.access_token)return[];return request("work_evidence",{query:`?deleted_at=is.null${workId?`&work_id=eq.${encodeURIComponent(workId)}`:""}&select=*&order=created_at.desc`});},create:createPreventiveEvidence,update:async(id,item)=>{requirePermission("evidence.upload");const rows=await request("work_evidence",{method:"PATCH",query:`?id=eq.${encodeURIComponent(id)}`,headers:{Prefer:"return=representation"},body:item});return Array.isArray(rows)?rows[0]:rows;}});
